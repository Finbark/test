...
class som:
    ...
    The original program has been modified to include additional functions to run a perceptron on the outputs from the SOM, see from line 94.    
    """
    ...
    def run_perceptron(self, data, data_targets, train_size, test_size):
        ...
        train_set_target, test_set_target = self.split_data(data_targets, train_size, test_size)
    ...
    def create_new_inputs(self, data):
        '''
        Creates the input to the perceptron by adding the SOM activations to a numpy array
        '''
        new_training_data = np.zeros((np.shape(data)[0], self.activations))
        for x in range(np.shape(data)[0]):
            best, new_training_data[x,:] = self.somfwd(data[x,:])
        return new_training_data
    
    def normalise(self, data):
        '''
        Normalisation of the activations using max and min.

        '''
        data = data-data.mean(axis=0)
        dmax = np.concatenate((data.max(axis=0)*np.ones((1,self.activations)),data.min(axis=0)*np.ones((1,self.activations))),axis=0).max(axis=0)
        data = data/dmax
    ...
    def split_data(self, data, train_size, test_size):
        '''
        Splits the initial data into training and testing sets.
        '''
